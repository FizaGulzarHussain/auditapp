...
def _build_category_block(cat_label: str, cat_key: str, cat_hex: str,
    ...
    if strengths:
        ...
        for s in strengths:
            ...
            str_group.append(Spacer(1, 3))
        block.append(KeepTogether(str_group))
...
def _story(audit: dict) -> list:
    ...
    for idx, (_, sev, cat_l, iss) in enumerate(all_ranked, 1):
        row = [
            ...
            Paragraph(f"<b>{_sev_label(sev)}</b>", st["Small"]),
            Paragraph(_safe(cat_l), st["Small"]),
            Paragraph(_safe(iss),   st["Small"]),
        ]
        tbl_data.append(row)
        _bg = colors.HexColor(
            "#FEF2F2" if sev == "CRITICAL" else
            "#FFFBEB" if sev == "HIGH"     else
            "#EFF6FF" if sev == "MEDIUM"   else "#F0FDF4"
        )
